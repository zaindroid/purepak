...
const API = (() => {
  ...
  return {
    ...
    backupList: () => g('/backup/list'),
    runBackup: () => post('/backup/run'),
    backupDownloadUrl: (filename) => '/api/backup/download/' + encodeURIComponent(filename) + '?token=' + encodeURIComponent(token || ''),
